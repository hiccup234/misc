...
/**
 * 【Netty实现IO的数据传输载体ByteBuf类】
 * <p>
 * Netty 使用了堆外内存（由具体的ByteBuf实现决定），需要我们手都分配和回收，通过引用计数的方式来管理内存的（实现ReferenceCounted接口）
 * 默认情况下，当创建完一个 ByteBuf对象，它的引用计数为1，然后每次调用 retain() 方法 -- 保持，
 * 它的引用就加一， release() 方法原理是将引用计数减一，减完之后如果发现引用计数为0，则直接回收 ByteBuf 底层的内存。
 * <p>
 * ================================================================================================
