...
/**
 * 【sleep】使当前线程休眠指定时间，不会释放锁，线程是变成阻塞状态（TIMED_WAITING）
 * 【wait】线程进入等待池，处于WAITING状态，会释放锁（必须在锁的同步块中调用）
 * 【yield】主动让出当前线程的CPU时间片，使正在运行中的线程重新变成就绪状态（RUNNABLE），并重新竞争CPU的调度权，避免一个线程长时间占有CPU资源，也不释放锁
 *
 ...
 * @date 2019/1/12
 */
public class SleepWaitYieldTest {
