 ...
 *       （这里锁标志位应该是跟锁记录指针一起CAS更新的，不然会有原子性问题）
 * 　　（5）如果更新失败（MarkWord已经被其他线程先一步修改了），虚拟机首先会检查对象的Mark Word是否指向当前线程的栈帧，
 *        这里为什么还要检查一次呢？ 注意：锁记录是分配在栈帧里而不是栈上的，同一个线程进入多个同步方法则会有多个栈帧多个锁记录，
 *        所以要检查是不是线程已经获取了锁，如果是就说明当前线程已经拥有了这个对象的锁，JVM会会把当前栈帧锁记录清零并直接进入同步代码块，
