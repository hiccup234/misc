...
/**
 * 参考JDK1.8（主要列出相对1.7变化的地方：Entry实现修改为Node）
 *
 * 1.当哈希表中链表长度超过默认的8（必须是2的n次方）时转换为红黑树，形成一个小型TreeMap
 *
 * @author wenhy
 * @date 2018/9/30
 */
public class MyHashMap8<K, V> extends AbstractMap<K, V>
    ...
    /**
     * TODO 跟1.7相比，哈希表存储的元素类型有HashMap.Entry变成了HashMap.Node
     *      主要修改了hashCode和equals方法的实现（改用Objects工具类方法），去掉了其他无关紧要的方法
     */
    ...
    /**
     * TODO 跟1.7相比，hash方法有了比较大的修改，且倍申明为了static
     */
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public final Iterator<Map.Entry<K, V>> iterator() {
            return new MyHashMap8.EntryIterator();
        }
        ...
        public final Spliterator<Map.Entry<K, V>> spliterator() {
            return new MyHashMap8.EntrySpliterator<>(MyHashMap8.this, 0, -1, 0, 0);
        }
    ...
    @Override
    public Object clone() {
        MyHashMap8<K, V> result;
        try {
            result = (MyHashMap8<K, V>) super.clone();
    ...
    static class HashMapSpliterator<K, V> {
        ...
        HashMapSpliterator(MyHashMap8<K, V> m, int origin,
                           int fence, int est,
    ...
    static final class KeySpliterator<K, V>
            extends MyHashMap8.HashMapSpliterator<K, V>
            implements Spliterator<K> {
        KeySpliterator(MyHashMap8<K, V> m, int origin, int fence, int est,
                       int expectedModCount) {
    ...
    static final class ValueSpliterator<K, V>
        ...
        ValueSpliterator(MyHashMap8<K, V> m, int origin, int fence, int est,
                         int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }

        public MyHashMap8.ValueSpliterator<K, V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyHashMap8.ValueSpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class EntrySpliterator<K, V>
        ...
        EntrySpliterator(MyHashMap8<K, V> m, int origin, int fence, int est,
                         int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }

        public MyHashMap8.EntrySpliterator<K, V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyHashMap8.EntrySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
         ...
         * this node.
         */
        final Node<K, V> untreeify(MyHashMap8<K, V> map) {
         ...
         */
        final void removeTreeNode(MyHashMap8<K, V> map, Node<K, V>[] tab,
                                  boolean movable) {
            int n;
