...
// TODO 1、HashMap中可以允许key和value同时为空（只能有一个为null的key，放在第一个桶中table[0]）
// TODO 3、动态扩容是把hash表的数组长度扩为原来的2倍（长度必须是2的x次方，方便hash定位）
// TODO 4、JDK1.8对hash表的链表长度默认超过8时（插入第9个元素）做了优化，改成红黑树来实现
// TODO 5、默认初始化容量（数组长度）为16，负载因子为0.75
// TODO 6、如果负载因子小于1，则Map的size永远小于哈希表的数组长度，（默认0.75的初衷就是空间换时间）
