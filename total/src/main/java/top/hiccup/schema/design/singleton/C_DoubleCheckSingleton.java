        ...
        if (null == instance) {
            synchronized (C_DoubleCheckSingleton.class) {
                if (null == instance) {
                    ...
                    // 如果用volatile来修饰则会产生写读屏障storeload，禁止2，3的指令重排，从而防止出现其他线程获取到未初始化完全的对象
                    // TODO 就算时间片用完切换其他线程也进入不了synchronized代码块，因为原来代码块还没执行完成，所以不会释放锁，也不会出现多次实例化（只是可能获取到未初始化完全的对象）
