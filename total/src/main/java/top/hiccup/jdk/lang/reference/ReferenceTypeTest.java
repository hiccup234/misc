 ...
 * 这就可以用来构建一种没有特定约束的关系，比如，维护一种非强制性的映射关系（如：WeakHashMap），
 ...
 * 幻像引用就是梦中情人，只在梦里出现过。
 * <p>
 * 软引用：图片缓存框架中，“内存缓存”中的图片是以这种引用来保存，使得JVM在发生OOM之前，可以回收这部分缓存。（Guava包的缓存是不是也是软引用？）
...
public class ReferenceTypeTest {
    ...
    private static void phantomReferenceTest() {
        ...
        try {
            // 幻象引用的get方法永远返回null
            System.out.println("P --> " + pRef.get());
            // remove 是一个阻塞方法，可以指定 timeout，或者选择一直阻塞（remove可以理解为获取的意思）
//            Reference<Object> qRef = refQueue.poll();
            Reference<Object> qRef = refQueue.remove();
            if (qRef != null) {
                System.out.println(pRef);
                System.out.println(qRef);
                System.out.println("pRef == qRef : " + (pRef == qRef));
            }
