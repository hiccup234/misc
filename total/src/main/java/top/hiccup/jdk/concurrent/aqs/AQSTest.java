 ...
 * PROPAGATE    -3  共享模式下使用，头节点获取资源时将后面节点设置为此状态，如果头节点获取资源后还有足够的资源，
 *                  则后面节点会尝试获取，这个状态主要是为了共享状态下队列里足够多的节点同时获取资源
 *              0   初始状态
 *
 * 有一个静态内部类Node节点，通过双向链表构成CLH队列，入队出队都是通过CAS替换的，所以多线程并发下不用锁来同步
 *
 * 详见AQS类的详细注释
 *
 * @author wenhy
