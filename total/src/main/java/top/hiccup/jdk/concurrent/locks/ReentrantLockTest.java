 ...
 *   ReentrantLock 是JDK的API，底层基于AQS，底层原理是cas，依赖于CPU的硬件支持（IA64、x86架构中汇编指令为：cmpxchg），
 *                  不会导致线程的阻塞和挂起，锁竞争激烈的情况下，性能表现更好
 *
 ...
 *      d、可绑定多个条件（Condition）
 *      e、可以判断某个线程是否在排队等待锁
 *      f、可以判断锁是否被占用，而synchronized无法直接做到（可以通过Unsafe判断对象头，也不好判断偏向锁的状态）
 *
 * =====================================================================================================================
