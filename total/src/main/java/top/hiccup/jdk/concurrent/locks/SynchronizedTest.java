 ...
 * synchronized底层原理：
 * =====================================================================================================================
 * 四种状态：无锁状态、偏向锁、轻量级锁和重量级锁（详见：Synchronized-LockType-Transform.jpg）
 *
 ...
 *
 * 同步块代码结束后不会自动释放偏向锁，如果有其他线程请求锁，才会触发偏向锁撤销（要在safe point），若偏向的线程还在执行临界区，
 * 则进行锁升级，若已退出临界区，则释放锁，恢复mark word为无锁状态，进行重偏向。
 *
 * 【轻量级锁】
 ...
 * 在linux上是使用pthread的mutex互斥锁，最底层实现依赖于futex，这些同步函数都涉及到用户态和内核态的切换、进程的上下文切换，开销成本较高。
 ...
 *        并将Lock record里的owner指针指向object mark word。更新成功，则执行步骤（4），否则（5）。
 * 　（4）如果更新成功，那么这个线程就拥有了该对象的锁，并且将对象Mark Word的锁标志位设置为“00”，即表示此对象处于轻量级锁定状态
 *       （注意：这里锁标志位应该是跟锁记录指针一起CAS更新的，不然会有原子性问题）
 * 　（5）如果更新失败（MarkWord已经被其他线程先一步修改了），虚拟机首先会检查对象的Mark Word是否指向当前线程的其他栈帧，
 *        这里为什么还要检查一次呢？ 注意：锁记录是分配在栈帧里而不是栈上的，同一个线程进入多个同步方法则会有多个栈帧多个锁记录，
 *        所以要检查是不是线程已经获取了锁，如果是就说明当前线程已经拥有了这个对象的锁，JVM会把当前栈帧锁记录清零并直接进入同步代码块，
 *        解锁时如果当前栈帧锁记录为0则直接退出（见极客时间手动拆解JVM14讲）
 *        否则说明有其他线程已经CAS竞争到锁，轻量级锁就要自旋膨胀为重量级锁，将锁标志的状态置为“10”，Mark Word中存储的就是指向重量级锁（互斥量）的指针，
 *        后面等待锁的线程也要进入阻塞状态，而当前线程便尝试使用自适应自旋来获取锁，自旋就是为了尽量不让线程阻塞（避免系统调用和上下文切换的开销），而采用循环去获取锁的过程。
 * 轻量级解锁过程：
 *   （1）通过CAS操作尝试把线程中复制的Displaced Mark Word对象替换到当前锁对象的Mark Word。
 * 　（2）如果替换成功，整个同步过程就完成了。
 * 　（3）如果替换失败，说明有其他线程尝试过获取该锁（此时锁已膨胀，所以MarkWord内容已不是本线程栈帧的锁记录，锁标志位变为10），
 *        需要在释放锁的同时，唤醒被挂起的线程。
 *
 * =====================================================================================================================
 * 【自旋锁】
 * 获取轻量级锁CAS失败后，JVM还会进行适应性的自旋，例如循环多次来CAS，如果都失败了再膨胀为重量级锁，并阻塞当前线程。
 * （自旋锁在单核CPU上是无效的，锁已经被其他线程占用了，如果没有线程切换，那么占用锁的线程就没法执行完临界区并释放锁）
 *
 * 【锁粗化】
